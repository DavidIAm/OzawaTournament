...
next unless scalar(keys %{$data->{teams}}) || scalar(grep { length $_ } map { values %{$_->{events}} } @{$data->{participants}});
  push @list, my $row = [ ($data->{paypal} ? 'paid' : 'unpaid' ) ];
  push @{$row}, $data->{paypal}{invoice}."<br>".scalar(localtime $_) || 0;
...
print "<style>td, th { background-color: white }
.paid td { background-color:#11FF11; }
.unpaid td { background-color:#FF7788; }
</style>\n";
